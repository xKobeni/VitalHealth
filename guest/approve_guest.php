...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($appointment_id) {
        ...
        $sql = "UPDATE guest_appointments SET status = 'scheduled' WHERE guest_appointment_id = ?";
        ...
        if ($stmt->execute()) {
            ...
            if ($appointment) {
                ...
                $subject = "Appointment Approved - VitalHealth";
                $message = "Dear " . $appointment['full_name'] . ",\n\n";
                $message .= "Your appointment with Dr. " . $appointment['doctor_name'] . " has been approved.\n";
                ...
                header('Location: ../doctordashboard.php?success=appointment_approved');
                exit;
            }
        }
    }
    
    header('Location: ../doctordashboard.php?error=approval_failed');
    exit;
} else {
