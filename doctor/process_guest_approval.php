...
$appointment_id = isset($_POST['appointment_id']) ? filter_input(INPUT_POST, 'appointment_id', FILTER_VALIDATE_INT) : 
                 (isset($_GET['appointment_id']) ? filter_input(INPUT_GET, 'appointment_id', FILTER_VALIDATE_INT) : null);
$action = isset($_POST['action']) ? filter_input(INPUT_POST, 'action', FILTER_SANITIZE_STRING) :
         (isset($_GET['action']) ? filter_input(INPUT_GET, 'action', FILTER_SANITIZE_STRING) : null);
$remark = isset($_POST['remark']) ? trim($_POST['remark']) : 
         (isset($_GET['remark']) ? trim($_GET['remark']) : null);
    ...
    if ($appointment) {
        if ($action === 'approve') {
            ...
            if ($update_stmt->execute()) {
                ...
                // Store email status in session instead of URL
            ...
            $update_sql = "UPDATE guest_appointments SET status = 'rejected', remark = ? WHERE guest_appointment_id = ?";
            $update_stmt = $conn->prepare($update_sql);
            $update_stmt->bind_param("si", $remark, $appointment_id);
